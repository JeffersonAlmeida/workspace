...
public class TCGRequirementsSelectionPage extends WizardPage implements TCGPage {
	...
	/**
	 * Sets the page title, description and indicates that it initially will not
	 * have the finish button enabled.
	 */
	public TCGRequirementsSelectionPage() {
		super(" ");
		setTitle(Properties
				.getProperty("test_suite_generation_by_requirements"));
		setDescription(Properties.getProperty("select_requirements_to_cover"));
	 ...
	 * 
	 * @param parent
	 *            The parent of the components that will be created.
	 * @return The main component of the page. It is used to set the control of
	 *         the wizard.
	 */
	...
	private void createRequirementsContainer(Composite parent) {
		...
		label.setText(Properties.getProperty("select_the_input_requirements"));
	...
	private void createButtonsContainer(Composite parent) {
		...
		this.selectAllButton.setText(Properties.getProperty("select_all"));
		...
		this.deselectAllButton.setText(Properties.getProperty("deselect_all"));
	 ...
	 * 
	 * @param flag
	 *            Indicates if the components have to be enabled or disabled.
	 */
	 ...
	 * 
	 * @param flag
	 *            Indicates if all requirements have to be enabled or disabled.
	 */
	 ...
	 * 
	 * @param composite
	 *            The parent component of the requirements table.
	 */
	private void createReqTable(Composite composite) {
		reqTable = new Table(composite, SWT.BORDER | SWT.CHECK
		...
		createTableColumn(reqTable, SWT.LEFT, Properties
				.getProperty("requirement_code"), 300);
	}
	 ...
	 * 
	 * @param table
	 *            The table in which the column will be created.
	 * @param style
	 ...
	 */
	private void createTableColumn(Table table, int style, String title,
			int width) {
	...
	public boolean anyRequirementSelected() {
		return getSelectedRequirements().size() > 0
				|| hasSelectedAllRequirements();
	}

	/**
	 * Checks if all requirements were selected. Checks if the number of
	 * selected requirements is the same of the requirements listed in
	 * <code>reqTable</code>.
	 * 
	 * @return <code>true</code> if all requirements were selected or
	...
	public boolean hasSelectedAllRequirements() {
		...
		return (map.size() == this.getSelectedRequirements().size())
				&& (map.size() != 0);
	}
	 ...
	 * 
	 * @param The
	 *            list of requirements that will be selected.
	 */
