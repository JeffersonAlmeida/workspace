...
public class TestCaseTable {
	...
	/**
	 * Creates grid component with the following style: SWT.BORDER |
	...
	private void createColumns(Grid grid) {
		...
		columns[0].setText(Properties.getProperty("case"));
		columns[1].setText(Properties.getProperty("reg_level"));
		columns[2].setText(Properties.getProperty("exe_type"));
		columns[3].setText(Properties.getProperty("case_description"));
		columns[4].setText(Properties.getProperty("procedure"));
		columns[5].setText(Properties.getProperty("expected_results"));
	 ...
	 * 
	 * @param testCase
	 *            The test case that will be exhibited.
	 */
	public void setTestCase(TextualTestCase testCase) {
		...
		if (testCase != null) {
			...
			item.setText(3, Properties.getProperty("use_cases"));
			item.setText(4, testCase.getUseCaseReferences());
			item.setHeight(this.computeRowHeight(item));

			item = this.createNewItem(this.testCaseGrid);
			item.setText(3, Properties.getProperty("requirements"));
			item.setText(4, testCase.getRequirements());
			item.setHeight(this.computeRowHeight(item));

			item = this.createNewItem(this.testCaseGrid);
			item.setText(3, Properties.getProperty("setup"));
			item.setText(4, testCase.getSetups());
			item.setHeight(this.computeRowHeight(item));

			item = this.createNewItem(this.testCaseGrid);
			item.setText(3, Properties.getProperty("initial_conditions"));
			item.setText(4, testCase.getInitialConditions());
			item.setHeight(this.computeRowHeight(item));

			item = this.createNewItem(this.testCaseGrid);
			item.setText(3, Properties.getProperty("notes"));
			item.setText(4, testCase.getNote());
			item.setHeight(this.computeRowHeight(item));

			item = this.createNewItem(this.testCaseGrid);
			item.setText(3, Properties
					.getProperty("test_procedure_step_mumber"));
			item.setHeight(this.computeRowHeight(item));
			...
			item = this.createNewItem(this.testCaseGrid);
			item.setText(3, Properties.getProperty("final_conditions"));
			item.setText(4, testCase.getFinalConditions());
			item.setHeight(this.computeRowHeight(item));

			item = this.createNewItem(this.testCaseGrid);
			item.setText(3, Properties.getProperty("cleanup"));
	...
	private int computeCellHeight(GridItem item, int column) {
		...
		currTextLayout.setWidth(Math.max(10, this.testCaseGrid
				.getColumn(column).getWidth() - 8));
	 ...
	 * 
	 * @param item
	 *            The GridItem which the font will be set.
	 */
	 ...
	 * 
	 * @param item
	 *            The GridItem which the background will be set
	 */
	private void setGreyArea(GridItem item) {
		item.setBackground(0, item.getDisplay().getSystemColor(
				SWT.COLOR_WIDGET_BACKGROUND));
		item.setBackground(1, item.getDisplay().getSystemColor(
				SWT.COLOR_WIDGET_BACKGROUND));
		item.setBackground(2, item.getDisplay().getSystemColor(
				SWT.COLOR_WIDGET_BACKGROUND));
	}
