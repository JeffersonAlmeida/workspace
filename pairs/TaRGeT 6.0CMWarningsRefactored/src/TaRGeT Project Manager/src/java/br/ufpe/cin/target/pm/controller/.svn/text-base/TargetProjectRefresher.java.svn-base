 ...
 * wmr068    Aug 07, 2008   LIBqq64190   Method reloadProject now calls ProjectManagerController.getUseCaseDocumentFiles() instead of FileUtil.getUseCaseDocumentFiles.
 */
...
import br.ufpe.cin.target.pm.errors.DifferentMergedFeatureNameError;
import br.ufpe.cin.target.pm.errors.DuplicatedFeatureIdInsideDocumentError;
import br.ufpe.cin.target.pm.errors.DuplicatedStepIdError;
import br.ufpe.cin.target.pm.errors.DuplicatedUseCaseIdError;
import br.ufpe.cin.target.pm.errors.DuplicatedUseCaseIdErrorBetweenDocuments;
...
import br.ufpe.cin.target.pm.errors.UnableToLoadDocumentError;
...
public class TargetProjectRefresher
{
            ...
            throws TargetException
    {
        ...
        Collection<UseCaseDocument> loadedDocuments = new ArrayList<UseCaseDocument>();
        ...
        this.verifyErrors(loadedDocuments, this.currentProject.getUseCaseDocuments(),
                throwExceptionOnFatalError);

        List<UseCaseDocument> allDocuments = new ArrayList<UseCaseDocument>();
        allDocuments.addAll(this.currentProject.getUseCaseDocuments());
        allDocuments.addAll(loadedDocuments);

        /* Verify errors if all the documents are imported */
        errors.addAll(this.verifyErrors(allDocuments, new ArrayList<UseCaseDocument>(), false));
    ...
    {
        Collection<UseCaseDocument> currentDocs = this.currentProject.getUseCaseDocuments();
        Collection<File> folderDocs = null;

        try
        {
            folderDocs = ProjectManagerController.getInstance().getUseCaseDocumentFiles();
        }
        catch (IOException e)
        {
            e.printStackTrace();
         ...
         * reload all documents, it is necessary to change the UseCaseDocument implementation, adding
         * the fatal error instance in the UseCaseDocument, and make a selection based on the errors.
         */
        ...
        Collection<UseCaseDocument> notLoadedDocuments = new ArrayList<UseCaseDocument>();
        for (UseCaseDocument curDoc : currentDocs)
        {
            boolean loaded = false;
            for (UseCaseDocument loadedDoc : loadedDocuments)
            {
         ...
         */
        errors.addAll(this.verifyErrors(allDocuments, new ArrayList<UseCaseDocument>(),
                throwExceptionOnFatalError));
        
        int nextID = 1;
        if(ProjectManagerController.getInstance().getCurrentProject() != null)
        {
            nextID = ProjectManagerController.getInstance().getCurrentProject().getNextAvailableTestCaseID();
        }

        /* Creates the new TargetProject with all processed documents */
        TargetProject newProject = new TargetProject(this.currentProject.getName(),
                this.currentProject.getRootDir(), allDocuments, nextID);
     ...
     * error.
     */
    private Collection<Error> loadDocuments(Collection<UseCaseDocument> loadedDocuments,
            ...
            throws TargetException
    {
        ...
        Collection<UseCaseDocument> justLoadedDocuments = controller.loadUseCaseDocumentFiles(docsToBeLoaded,
                throwExceptionOnError);

        /* Creating the error instances and adding the not loaded documents */
        for (UseCaseDocument pDoc : justLoadedDocuments)
        {
     ...
     * encountered.
     */
    private Collection<Error> verifyErrors(Collection<UseCaseDocument> newDocuments,
            Collection<UseCaseDocument> currentDocuments, boolean throwExceptionOnFatalError)
            throws TargetProjectLoadingException
    {
        /* A new Collection is created to not change the currentDocuments */
        currentDocuments = new ArrayList<UseCaseDocument>(currentDocuments);
        /* A clone collection is created. This collection will not be changed */
        Collection<UseCaseDocument> processedDocuments = new ArrayList<UseCaseDocument>();

        Collection<Error> result = new ArrayList<Error>();
        
        /* the for run over the clone collection */
        for (UseCaseDocument pDoc : newDocuments)
        {
            if (pDoc.isDocumentWellFormed())
            {
            	List<Error> errors = verifyUseCaseDocument(pDoc, currentDocuments, throwExceptionOnFatalError);
    ...
    public List<Error> verifyUseCaseDocument(UseCaseDocument pDoc, Collection<UseCaseDocument> currentDocuments, boolean throwExceptionOnFatalError) throws TargetProjectLoadingException
    {
        ...
        if (!errors.isEmpty() && this.hasFatalErrors(errors))
        {
            ...
            pDoc = new UseCaseDocument(pDoc.getDocFilePath(), pDoc
     ...
     */
    private Collection<Error> getEmptyFieldErrors(Collection<UseCaseDocument> documents)
    {
        Collection<Error> result = new ArrayList<Error>();

        for (UseCaseDocument pDoc : documents)
        {
            if (pDoc.isDocumentWellFormed())
            {
     ...
     /**
     * Verifies the documents in the parameter <code>useCaseDocuments</code>, and returns a
     ...
     */
    private Collection<String> getFatalErrorDocuments(Collection<UseCaseDocument> useCaseDocuments)
    {
        List<String> result = new ArrayList<String>();

        for (UseCaseDocument pDoc : useCaseDocuments)
        {
     ...
     * modification date.
     * 
     * @param useCaseDocuments The documents to be verified.
     ...
     */
    private Collection<String> getChangedDocuments(Collection<UseCaseDocument> useCaseDocuments,
    ...
    private Collection<String> getRemovedDocuments(Collection<File> folderFiles)
    {
        ...
        for (String filePath : this.currentProject.getUseCaseDocumentFilePaths())
        {
     ...
     */
    private Collection<String> getAddedDocuments(Collection<UseCaseDocument> useCaseDocuments,
            Collection<File> folderFiles)
    {
        List<String> result = new ArrayList<String>();
     ...
     */
    private List<Error> verifyStepIdErrors(Collection<UseCaseDocument> documentList)
    {
        ...
        for (UseCaseDocument pDoc : documentList)
        {
            if (pDoc.isDocumentWellFormed())
            {
                for (Feature feature : pDoc.getFeatures())
     ...
     * 
     * @param useCaseDocument The document to be verified
     * @return The list that contains the found duplicity errors
     */
    private List<Error> verifyDuplicatedUseCaseIds(UseCaseDocument useCaseDocument)
    {
        List<Error> result = new ArrayList<Error>();

        for (Feature feature : useCaseDocument.getFeatures())
        {
            ...
            {
                result
                        .add(new DuplicatedUseCaseIdError(feature, uc, useCaseDocument
                                .getDocFilePath()));
            }
        }
     ...
     * 
     * @param documentList The list of documents which useCaseDocument will be verified against.
     * @param useCaseDocument The document to be verified.
     * @return The list that contains the found duplicity errors
     */
    private List<Error> verifyDuplicatedUseCaseIds(Collection<UseCaseDocument> documentList,
            UseCaseDocument useCaseDocument)
    {
        ...
        for (UseCaseDocument pd : documentList)
        {
            if (pd.isDocumentWellFormed())
            {
                ...
                for (Feature feature : pdFeatures)
                {
                    ...
                    if (map.get(feature.getId()) != null)
                    {
                        ...
                        Feature duplicatedFeature = useCaseDocument.getFeature(feature.getId());
                                ...
                                feature, duplicatedFeature)).isEmpty())
                        {
                            ...
                            result.add(new DuplicatedUseCaseIdErrorBetweenDocuments(feature,
                                    usecases.iterator().next(), pd.getDocFilePath(), useCaseDocument
                                            .getDocFilePath()));
                        }
                    }
                }
     ...
     * 
     * @param useCaseDocument The document to be verified the feature id duplicity
     * @return A list that contains the found duplicity errors
     */
    private List<Error> verifyDuplicatedFeatureId(UseCaseDocument useCaseDocument)
    {
        List<Error> result = new ArrayList<Error>();
        ...
        for (Feature feature : useCaseDocument.getFeatures())
        {
            ...
            if (num != null)
            {
                ...
                result.add(new DuplicatedFeatureIdInsideDocumentError(feature, useCaseDocument
                        .getDocFilePath()));
            }
            else
            {
     ...
     * @param useCaseDocument The document to be verified the feature name difference.
     * @return A list of errors.
     */
    private List<Error> verifyDifferentMergedFeatureName(Collection<UseCaseDocument> documentList,
            UseCaseDocument useCaseDocument)
    {
        List<Error> result = new ArrayList<Error>();
        documentList = new ArrayList<UseCaseDocument>(documentList);

        List<Feature> useCaseDocumentFeatures = useCaseDocument.getFeatures();

        HashMap<String, Integer> map = new HashMap<String, Integer>();
        for (Feature feature : useCaseDocumentFeatures)
        {
            Integer num = map.get(feature.getId());
            map.put(feature.getId(), (num != null) ? (num.intValue() + 1) : 1);
        }
                    ...
                    if (map.get(feature.getId()) != null)
                    {
                                ...
                                && !feature.getName().equals(duplicatedFeature.getName()))
                        {
                            ...
                            result.add(new DifferentMergedFeatureNameError(duplicatedFeature,
                                    useCaseDocument.getDocFilePath()));
                            useCaseDocumentFeatures.remove(duplicatedFeature);
                        }
                    }
                }
            }
