 ...
 * fsf2		Jun 20, 2009				 Creation of method getFilterExtensions.
 */
 ...
 * COLABORATORS:
 * 1) References
 * <code>DocumentsSelectionPage</code>
 */
public class DocumentsSelectionEventHandler extends SelectionAdapter {
	...
	/**
	 * Handles the widget selection event. This method handles all the events
	 * regarding the document selection (E.g. Adding or removing documents).
	 * 
	 * @param e
	 *            The selection event.
	 */
	public void widgetSelected(SelectionEvent e) {
		...
		if (e.getSource() == this.documentsSelectionPage.getDocumentList()) {
			boolean condition = (this.documentsSelectionPage.getDocumentList()
					.getItemCount() > 0 && this.documentsSelectionPage
					.getDocumentList().getSelectionCount() > 0);
			/*
			 * enables the remove button if there is at least one feature and
			 * one selected
			 */
		...
		/* checks if the event is related to a click on the add button */
			...
			if (browsedDocuments.length > 0) {
				String[] existing = this.documentsSelectionPage
						.getDocumentList().getItems();

				for (String document : browsedDocuments) {
					// Verifies if the selected files is already inserted in the
					// documents to import
					// list
					boolean found = false;
					for (String string : existing) {
						if (FileUtil.getFileName(string).toLowerCase()
								.equalsIgnoreCase(
										FileUtil.getFileName(document)
												.toLowerCase())) {
							found = true;
							break;
						}
					...
					if (!found) {
						this.documentsSelectionPage.getDocumentList().add(
								document);
		...
		/* checks if the event is related to a click on the remove button */
			...
			if (selectedIndex.length > 0) {
				this.documentsSelectionPage.getDocumentList().remove(
						selectedIndex);
				this.documentsSelectionPage.getRemButton().setEnabled(false);
				/*
				 * disables the finish button if there are no features
				 * (documents) in the list
				 */
				this.documentsSelectionPage
						.setPageComplete(this.documentsSelectionPage
	...
	/**
	 * Starts a browser dialog that allow the user to select a document from
	 * file system.
	 * 
	 * @return The path of the selected file or <code>null</code> if the dialog
	 *         was canceled.
	 */
	private String[] handleDocumentBrowse() {
		FileDialog dialog = new FileDialog(this.documentsSelectionPage
				.getShell(), SWT.MULTI);
	...
	/**
	 * Gets the available input filter extensions (i.e. *.doc, *.xml etc) and
	 * the respective names of each filter extension. The filter extensions are
	 * stored in the first row of a resulting matrix whereas the names are
	 * stored in the second row.
	 * 
	 * @return A matrix containing the available input filter extensions and
	 *         their names.
	 */
