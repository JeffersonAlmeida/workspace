...
import com.motorola.btc.research.target.tcg.extensions.ConsistencyManagerExtensionFactory;
...
public class OnTheFlyGeneratedTestCasesPage extends FormPage
{
    ...
    /**
     * Check button that defines whether the consistency management shall be started or not before
     * saving test cases.
     */
    // INSPECT
    ...
    private void createGeneratedTestCasesSection(final ScrolledForm form, FormToolkit toolkit)
    {
        ...
        if (cm != null)
        {
            ...
            consistencyManagementcheckButton.setText("Start Consistency Management before saving.");
        }
            ...
            public void widgetSelected(SelectionEvent e)
            {
                ...
                try
                {
                    ConsistencyManagerExtension cmExtension = ConsistencyManagerExtensionFactory
                            .newConsistencyManagerExtension();
                    if (cmExtension != null && consistencyManagementcheckButton.getSelection())
                    {
                        cmExtension.openConsistencyManager(getEditor().getSite().getShell(),
                                textualTestCases);
                    }
                    else
                    {
                        ...
                        TestCaseGeneratorController.getInstance().writeTestSuiteFile(
                                new TestSuite<TextualTestCase>(textualTestCases, testSuite
                                        .getName()));
    ...
    public void removeTestCases(TextualTestCase testCase)
    {
        ...
        {
            if (this.generatedTestCasesTable.getItem(i).getData().equals(testCase))
            {
