 ...
 * fsf2		 20/06/2009					 Inclusion of a hidden field to store the test case id.
 */
...
public class ExcelExtractor implements ExtractorDocumentExtension
{
            ...
            IOException
    {
        ...
        while (!this.excelExtractor.finished()
                && (this.excelExtractor.getCellType(0) == br.ufpe.cin.target.tcg.util.ExcelExtractor.CELL_TYPE_NUMERIC || 
                		(this.excelExtractor.getCellString(0).matches(".*\\d+"))))
        {
            int id = getId();
    ...
    private int getId()
    {
        ...
        if (this.excelExtractor.getCellType(6) == br.ufpe.cin.target.tcg.util.ExcelExtractor.CELL_TYPE_STRING)
        {
            try
            {
                retorno = Integer.parseInt(this.excelExtractor.getCellString(6));
            }
    ...
    private String getIdHeader()
    {
        ...
        if (this.excelExtractor.getCellType(0) == br.ufpe.cin.target.tcg.util.ExcelExtractor.CELL_TYPE_STRING)
        {
            strId = this.excelExtractor.getCellString(0);
        }
        else
        {
            strId = String.valueOf(this.excelExtractor.getColumnIndex(0));
        }
    ...
    {
        return this.excelExtractor.getCellString(1);
    ...
    {
        String objective = this.excelExtractor.getCellString(4);
    ...
    {
        String useCaseReferences = this.excelExtractor.getCellString(4).trim();
    ...
    {
        String requirements = this.excelExtractor.getCellString(4).trim();
    ...
    {
        String setups = this.excelExtractor.getCellString(4).trim();
    ...
    {
        String initialConditions = this.excelExtractor.getCellString(4).trim();
    ...
    {
        String note = this.excelExtractor.getCellString(4).trim();
    ...
    private List<TextualStep> getSteps()
    {
        ...
        while (this.excelExtractor.getCellType(3) == br.ufpe.cin.target.tcg.util.ExcelExtractor.CELL_TYPE_NUMERIC
                || this.excelExtractor.getCellString(3).trim().matches("\\d+"))
        {
            String action = this.excelExtractor.getCellString(4).trim();
            String response = this.excelExtractor.getCellString(5).trim();
    ...
    {
        String finalConditions = this.excelExtractor.getCellString(4).trim();
    ...
    {
        String cleanup = this.excelExtractor.getCellString(4).trim();
