...
import br.ufpe.cin.target.pm.ReloadProjectShellListener;
import br.ufpe.cin.target.pm.common.TargetTreeView;
import br.ufpe.cin.target.pm.common.TreeObject;
import br.ufpe.cin.target.pm.common.TreeViewContentProvider;
import br.ufpe.cin.target.pm.controller.ProjectManagerController;


/**
 * Represents the view that displays the documents tree and the test suites
 * tree.
 * 
 * <pre>
 * CLASS:
...
public class ArtifactsView extends TargetTreeView {
	 ...
	 */
	public static final String ID = "br.ufpe.cin.target.pm.views.artifacts.ArtifactsView";
	...
	/**
	 * Default constructor. Initializes the parent nodes and the double click
	 * action to null.
	 */
	...
	/**
	 * Implements the hook method from the superclass. In this class it adds a
	 * shell listnener to the main shell.
	 */
	...
	protected void hook() {
		...
		if (shell != null) {
			if (shellListener == null) {
				...
				shell.addShellListener(shellListener); // adds the listener to
														// the parent shell.
			}
		}
	}
	...
	/**
	 * Creates the content provider that handles the nodes. It initializes the
	 * root nodes.
	 * 
	 * @param labels
	 *            The labels to be added to the trees.
	 * @param type
	 *            The type of the labels.
	 * @return The content provider
	 */
	private TreeViewContentProvider createContentProvider(
			final Collection<String> labels, final DocumentType type) {
		return new TreeViewContentProvider() {
			...
			public void initialize() {
				if (type == DocumentType.useCase) {
					...
					// add the test suites
					Collection<String> testSuites = ProjectManagerController
							.getInstance().getTestSuiteDocuments();
	...
	/**
	 * Fills the <code>featureNode</code> with the features. Iterates over the
	 * features and adds them as children of the feature node.
	 * 
	 * @param featureNode
	 *            The node to be filled.
	 * @param features
	 *            The features that will fill the node.
	 */
	private void fillFeatureNode(TreeObject featureNode,
			Collection<String> features) {
		for (String feature : features) {
			String featureName = FileUtil.getFileName(feature);
			if (!this.existChild(featureName, featureNode)) {
				DocumentTreeObject tdo = new DocumentTreeObject(featureName,
						feature, DocumentType.useCase);
	...
	/**
	 * Fills the test case node. Iterates over the test suites to add them as
	 * children of the test case node.
	 * 
	 * @param testCaseNode
	 *            The node to be filled with the test cases.
	 * @param testSuites
	 *            The test suites that will fill the node.
	 */
	private void fillTestCaseNode(TreeObject testCaseNode,
			Collection<String> testSuites) {
		for (String testSuite : testSuites) {
			String suiteName = FileUtil.getFileName(testSuite);
			if (!this.existChild(suiteName, testCaseNode)) {
				DocumentTreeObject tdo = new DocumentTreeObject(suiteName,
						testSuite, DocumentType.testCase);
	...
	/**
	 * Make the double click action. When a user make a double-click in feature
	 * document or test suite document, an appropriate editor (MSWord or Excel)
	 * is opened. Note, a new implementation of Action is developed.
	 */
	
	protected void makeActions() {
		this.doubleClickAction = new DoubleClickAction(this.viewer);

		this.renameDocumentAction = new RenameDocumentAction(this.parent,
				this.viewer, this.featureNode, this.testCaseNode);

		this.deleteDocumentAction = new DeleteDocumentAction(this.parent,
				this.viewer);
	}
	 ...
	 * 
	 * @param manager
	 *            The menu manager to which the actions will be added to.
	 */
	...
	/**
	 * This method is used to update the tree viewer with feature documents and
	 * test suites. It avoids duplications in the tree. It first checks if
	 * exists a featureNode. If it exists then it verifies if the features are
	 * not null and if exists at least one of them. If the last condition is
	 * verified the it instantiates a new content provider.
	 */
	private void updateDocuments() {
		Collection<String> files = ProjectManagerController.getInstance()
				.getCurrentProject().getUseCaseDocumentFilePaths();
	...
	private void updateTestSuites() {
		Collection<String> names = ProjectManagerController.getInstance()
	 ...
	 * 
	 * @param labels
	 *            The labels to be added to the content provider.
	 * @param type
	 *            The type of the labels.
	 */
	private void initializeContentProvider(Collection<String> labels,
			DocumentType type) {
	 ...
	 * 
	 * @param root
	 *            The root in which the children will be searched.
	 * @param names
	 *            The existing names.
	 */
	 ...
	 * @param names
	 *            The names where <code>name</code> will be searched.
	 * @return <i>True</i> if the name is in <code>names</code> or <i>false</i>
	 *         otherwise.
	 */
