 ...
 * example,
 * <p>
 * If s is "test" and t is "test", then LD(s,t) = 0, because no transformations are needed and the
 * strings are already identical.<br/> If s is "test" and t is "tent", then LD(s,t) = 1, because
 * one substitution (change "s" to "n") is sufficient to transform s into t. <br/>
 * <p>
...
public final class LevenshteinDistance
{
    ...
    public double levenshteinDistance(String s, String t)
    {
        return (levenshteinDistance(s.toCharArray(), t.toCharArray(),
                s.toLowerCase().toCharArray(), t.toLowerCase().toCharArray(), false)) / 1000.0;
    }

    /**
     * A modified Levenshtein distance for any given two <code>String</code> objects. It is noted
     * whether there is any case difference, an extra .5 is added to the distance. This slighly
     * penalizes for case differences, rather than giving a severe penalization when each character
     * case difference is counted as a full transformation.
     * 
     * @param s Source <code>String</code>.
     * @param t Target <code>String</code>.
    ...
    {
        return (levenshteinDistance(s.toCharArray(), t.toCharArray(),
                s.toLowerCase().toCharArray(), t.toLowerCase().toCharArray(), true)) / 1000.0;
    }

    /**
     * This method returns a modified Levenshtein distance for any given two <code>String</code>
     * objects. It is noted whether there is any case difference, an extra .5 is added to the
     * distance. This slighly penalizes for case differences, rather than giving a severe
     * penalization when each character case difference is counted as a full transformation.
     * <p>
     * The char arrays are being passed in to speed things up. Also, the lowercase char arrays are
     * being passed in to speed things up. It was observed that in a typical invocation of this
     * method, this method gets called over and over again, with one of the strings being held
     * constant, and one of them changing. By being able to pass in the char arrays, I can convert
     * the string that remains constant only once to a char array, and lowercase it only once. I can
     * do array indexing rather than method invocation which should speed things up a bit as well.
     * <p>
     * To futher the efficiencies, this method returns an int of the distance that is the distance
     * multiplied by 1000. Thus, an edit distance of 2 multiplied by 1000 = 2000;
     * 
     ...
     * @param useCaseDifference If true, than case differences result in an extra .5 added to the
     * distance
     * @return The modified Levenshtein distance multiplied by 1000.
     */
    public int levenshteinDistance(char[] s, char[] t, char[] lowercaseS, char[] lowercaseT,
        ...
        System.out
                .println(l
                        .modifiedLevenshteinDistance(
                                "A dialog is disPlayed informing that a license is required to use the content and asking for license acquisition.",
                                "A dialog is displayed informing the license was received and asking the user to play the file."));
    }
