...
import br.ufpe.cin.target.tcg.wizards.TCGRequirementsSelectionPage;
import br.ufpe.cin.target.tcg.wizards.TCGUseCaseSelectionPage;
import br.ufpe.cin.target.tcg.wizards.TestPurposeCreationPage;
...
public class OnTheFlyTestSelectionPage extends FormPage {
	 ...
	 */
	public static final String ID = "br.ufpe.cin.target.tcg.editors.OnTheFlyTestSelectionPage";
	...
	public OnTheFlyTestSelectionPage(FormEditor editor) {
		super(editor, ID, Properties.getProperty("select_your_test_cases"));
	}
	...
	public TestSuiteFilterAssembler getFilterAssembler() {
		...
		if (this.requirementsSelectionPage != null
				&& this.useCaseSelectionPage != null
				&& this.testPurposePage != null) {
			result.setRequirementsFilter(this.requirementsSelectionPage
					.getSelectedRequirements());
			result.setUseCasesFilter(this.useCaseSelectionPage
					.getSelectedUseCases());
			result.setPurposeFilter(this.testPurposePage.getPurposesList());
			try {
				result.setSimilarityFilter(this.testPurposePage
						.getPathCoverage());
			} catch (InvalidSimilarityException e) {
				e.printStackTrace();
				TCGActivator.logError(0, this.getClass(), Properties
						.getProperty("invalid_similarity_value")
	...
	public void setFilterAssembler(TestSuiteFilterAssembler assembler) {
		if (assembler.getRequirementsFilter() != null) {
			...
			this.requirementsSelectionPage
					.setSelectedRequirements(new HashSet<String>());
		}

		if (assembler.getUseCasesFilter() != null) {
			this.useCaseSelectionPage.setSelectedUseCases(assembler
					.getUseCasesFilter().getUsecases());
		} else {
			this.useCaseSelectionPage
					.setSelectedUseCases(new HashMap<Feature, List<UseCase>>());
		}

		if (assembler.getSimilarityFilter() != null) {
			this.testPurposePage.setPathCoverage((int) assembler
					.getSimilarityFilter().getSimilarity());
		} else {
			this.testPurposePage.setPathCoverage(100);
		}

		if (assembler.getPurposeFilter() != null) {
			this.testPurposePage.setPurposeList(assembler.getPurposeFilter()
					.getPurposes());
		} else {
	 ...
	 * 
	 * @param feature
	 *            The feature that contains the use case.
	 * @param usecase
	 *            The use case that will be selected.
	 */
	 ...
	 *            The feature that contains the use case.
	 * @param usecase
	 *            The use case that will be verified.
    ...
    {
        return this.useCaseSelectionPage.getSelectedUseCases().isEmpty()
                || this.useCaseSelectionPage.isUseCaseSelected(feature, usecase);
    }
	 ...
	 * 
	 * @param requirement
	 *            The requirement that will be selected.
	 */
	public void selectRequirement(String requirement) {
		Set<String> requirements = this.requirementsSelectionPage
				.getSelectedRequirements();
	 ...
	 * 
	 * @param requirement
	 *            The requirement that will be verified.
	 * @return True if the requirement exists and it is selected, false
	 *         otherwise.
	 */
    public boolean isRequirementSelected(String requirement)
    {
        return this.requirementsSelectionPage.getSelectedRequirements().isEmpty()
                || this.requirementsSelectionPage.getSelectedRequirements().contains(requirement);
    }
	...
	protected TestSuite<TestCase<FlowStep>> getCurrentTestSuite() {
		...
		return getFilterAssembler().assemblyFilter().filter(
	 ...
	 */
	private void createPurposeSelectionPage(final ScrolledForm form,
			FormToolkit toolkit) {
		...
		section.setText(Properties.getProperty("test_purpose_creation"));
		section.setDescription(Properties
				.getProperty("create_your_test_purposes"));
	 ...
	 *            The toolkit used by this form.
	 */
	private void createRequirementAndUseCaseSelectionPage(
		...
		section.setText(Properties
				.getProperty("requirements_and_use_cases_selection"));
		section
				.setDescription(Properties
						.getProperty("select_the_requirements_and_use_cases_you_want_to_cover"));
		section.setClient(client);
		section.setExpanded(true);
