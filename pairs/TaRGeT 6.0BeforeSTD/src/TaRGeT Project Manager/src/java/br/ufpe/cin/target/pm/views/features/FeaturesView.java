...
public class FeaturesView extends TargetTreeView {
	 ...
	 */
	public static final String ID = "br.ufpe.cin.target.pm.views.features.FeaturesView";

	/**
	 * The default action that is responsible for opening an use case inside the
	 * tool.
	 */
	 ...
	 * 
	 * @param The
	 *            menu manager to which the actions will be added to.
	 */
	...
	protected void makeActions() {
		defaultViewAction = new Action() {
			public void run() {
				IStructuredSelection selection = (IStructuredSelection) viewer
						.getSelection();
				if (selection.size() == 1) {
					TreeObject object = (TreeObject) selection
							.getFirstElement();
					if (!object.hasChildren()) {
		...
		defaultViewAction.setText(Properties.getProperty("default_view"));
		...
		htmlViewAction.setText(Properties.getProperty("default_browser"));
	}
	...
	/**
	 * Opens the use case in a browser or in default view. Depending on the type
	 * of the view the method call HTMLGenerator.openUseCaseInsideTool() or
	 * HTMLGenerator.openUseCaseInBrowser(). Both the selected feature and
	 * usecase are retrieved from the selected TreeObject.
	 * 
	 * @param view
	 *            The enum used to retrieve the types of views.
	 */
	public void openUseCase(ViewEnum view) {
		...
		if (!treeObject.equals(this.featuresRoot) && !treeObject.hasChildren()) {
			...
			Feature selectedFeature = (Feature) treeObject.getParent()
					.getValue();
			try {
				if (selectedUseCase != null) {
					List<br.ufpe.cin.target.pm.errors.Error> errors = ProjectManagerController
							.getInstance().getErrorList();
					UseCaseHTMLGenerator generator = new UseCaseHTMLGenerator();
					if (view.equals(ViewEnum.defaultView)) {
						generator.openUseCaseInsideTool(selectedUseCase,
								selectedFeature, errors, false);
					} else {
						generator.openUseCaseInBrowser(selectedUseCase,
								selectedFeature, errors);
					}
				}
			} catch (Exception e) {
				showMessage(Properties.getProperty("error_opening_use_case")
						+ " " + e.getMessage());
			}
		}
	}
	...
	private void showMessage(String message) {
		MessageDialog.openInformation(viewer.getControl().getShell(),
				Properties.getProperty("requirements"), message);
	}
	...
	/**
	 * Updates the use cases according to the <code>features</code> passed as
	 * parameters.
	 */
	private void updateViewContents() {
		...
		if (this.contentProvider == null) {
			...
			if ((features != null && features.size() > 0)) {
				...
				this.viewer
						.setLabelProvider(new FeaturesTreeViewLabelProvider());
	 ...
	 * 
	 * @param features
	 *            The features to be added to the trees.
	...
	private TreeViewContentProvider createContentProvider(
		...
		return new TreeViewContentProvider() {
			...
			public void initialize() {
				featuresRoot = new TreeObject(Properties
	 ...
	 */
	public void fillFeatureRoot(TreeObject root, Collection<Feature> features) {
	...
	private void refreshRemovedFeatures() {
		if (this.featuresRoot != null) {
			Collection<Feature> features = ProjectManagerController
					.getInstance().getCurrentProject().getFeatures();
	...
	public void setFeaturesRoot(TreeObject treeObject) {
		this.featuresRoot = treeObject;
	}

	public TreeObject getFeaturesRoot() {
		return this.featuresRoot;
	}
