...
/**
 * This indexer is responsible for indexing contents in PhoneDocuments for later search.
 * 
 * <pre>
 * CLASS:
 ...
 * it in the index. It basically extracts the relevant data from a phone document and creates the
 * Lucene document objects. 
 * 
 * Managed operations over the index:
 * <ol>
 *  <li>indexing phone documents
 *  <li>removing phone documents
...
class Indexer
{
    ...
    /**
     * Index a collection of phone documents and stores in the index directory. This method checks
            ...
            throws TargetSearchException
    {
        if (documents != null && documents.size() > 0)
        {
            try
            {
                ...
                indexPhoneDocuments(documents, writer);
            ...
            catch (IOException e)
            {
                ...
                TargetSearchException sException = new TargetSearchException(
                        "Error indexing phone documents.");
     ...
     * index.
     */
    private void indexPhoneDocuments(Collection<PhoneDocument> documents, IndexWriter writer)
            throws TargetSearchException
    {
        try
        {
            for (PhoneDocument document : documents)
            {
