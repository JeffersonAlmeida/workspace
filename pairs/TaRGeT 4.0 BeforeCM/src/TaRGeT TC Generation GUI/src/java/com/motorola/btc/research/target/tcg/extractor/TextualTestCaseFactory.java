 ...
 * fsf2      Fev 19, 2009    LIBqq51204   Adapt to Test Central 4 template (status, regression level)
 */
...
public class TextualTestCaseFactory
{
	 ...
	 * @param documentsFlows The phone documents flows from which the test cases were extracted
	 * @return
	 */
			...
			List<Flow> documentsFlows)
	{
		...
		String strId = getDescription(rawTest);

		String tcID = createTCId(rawTest);
		...
		TextualTestCase textualTest = new TextualTestCase(rawTest.getId(),tcID+rawTest.getId(), steps, executionType,
				regLevel, strId, objective, strRequirements, setups, initialConditions, note,
				finalConditions, cleanup, strUseCases,status);
		return textualTest;
	}
	...
	private  String createTCId(TestCase<FlowStep> rawTest)
	{
		String result = TEST_CASE_ID;

		int id = rawTest.getId() - 1 + TEST_CASE_INITIAL_ID ;

		result = result.replaceAll("<tc_id>", String.valueOf(id));
		result = result.replaceAll("<tc_featureid>", getDefiningFeature(rawTest));

		return result;
	}

	private String getDescription(TestCase<FlowStep> rawTest)
	{
		return checkPrintUseCaseDescriptionProperty(rawTest);
	}
	...
	private  String getSetups(TestCase<FlowStep> rawTest, List<String> usecaseReferences)
	{
		...
		for (FlowStep step : rawTest.getSteps())
		{
			String setup = step.getSetup();
