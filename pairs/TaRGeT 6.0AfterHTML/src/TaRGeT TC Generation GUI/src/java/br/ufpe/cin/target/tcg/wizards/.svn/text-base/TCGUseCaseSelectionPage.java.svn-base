...
public class TCGUseCaseSelectionPage extends WizardPage implements TCGPage {
	...
	public TCGUseCaseSelectionPage() {
		super("");
		setTitle(Properties.getProperty("test_suite_generation_wizard"));
		setDescription(Properties
				.getProperty("select_the_use_cases_to_cover_or_generate_all_tests"));
	 ...
	 * 
	 * @param parent
	 *            The parent of all components to be created.
	 */
	 ...
	 * 
	 * @param parent
	 *            The parent of the tree to be created.
	 */
	private void createTree(Composite parent) {
		...
		group.setText(Properties.getProperty("available_use_cases"));
	 ...
	 * 
	 * @param parent
	 *            The parent of the table to be created.
	 */
	private void createUseCasesTable(Composite parent) {
		...
		GridData gridData = new GridData(GridData.FILL_BOTH);
		group.setLayoutData(gridData);
		group.setText(Properties.getProperty("selected_use_cases"));

		this.usecasesTableViewer = new TableViewer(new Table(group, SWT.MULTI
	...
	/**
	 * Creates the add buttons and remove button. It also creates their
	 * listeners.
	 * 
	 * @param parent
	 *            The parent of the buttons to be created.
	 */
	private void createButtons(Composite parent) {
		...
		addUseCaseButton.setText(Properties.getProperty("add_use_case"));
					...
					while (iterator.hasNext()) {
						...
						if (object instanceof TreeObject) {
							...
							if (treeObject.getValue() instanceof UseCase) {
								UseCase useCase = (UseCase) treeObject
										.getValue();
								Feature parentFeature = (Feature) treeObject
										.getParent().getValue();
								List<UseCase> usecases = selectedUseCases
										.get(parentFeature);
								if (usecases == null) {
								...
								if (!usecases.contains(useCase)) {
									...
									selectedUseCases.put(parentFeature,
											usecases);
		...
		removeUseCaseButton.setText(Properties.getProperty("remove_use_case"));
	 ...
	 * 
	 * @param useCase
	 *            The use case that will be removed.
	 */
	 ...
	 * 
	 * @param parent
	 *            The parent component of the page.
	 */
	 ...
	 */
	private class UseCasesLabelProvider extends LabelProvider implements
			ITableLabelProvider {
	...
	/**
	 * Returns the feature from the given <code>usecase</code> in the set of
	 * selected use cases.
	 * 
	 * @param usecase
	 *            The selected use case.
	 * @return The feature from the given <code>usecase</code> or null if the
	...
	/**
	 * Return objects representing the feature and the use case given as
	 * parameters.
	 * 
	 * @param feat
	 *            The identifier for the feature of the use case.
	 * @param uc
	 *            The identifier for the use case that will be selected.
	...
	/**
	 * Sets the use cases selection according to the list of use cases. These
	 * use cases will be in the use cases filter.
	 * 
	 * @param useCases
	 *            A map with features and the use cases that will be selected.
	 */
